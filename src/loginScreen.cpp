...
void LoginScreen::setupUI() {
  ...
  usernameInput->setStyleSheet("background-color: #323547; border: none; padding: 10px; border-radius: 5px;");
  ...
  loginButton->setStyleSheet("background-color: #5c6894; border: none; padding: 10px; border-radius: 5px;");
  ...
  connect(togglePasswordButton, &QPushButton::clicked, this, &LoginScreen::togglePasswordVisibility);
}
...
void LoginScreen::showInputFields() {
  ...
  QGraphicsOpacityEffect *containerEffect = new QGraphicsOpacityEffect(inputContainer);
  inputContainer->setGraphicsEffect(containerEffect);

  // Create opacity animation
  QPropertyAnimation *animation = new QPropertyAnimation(containerEffect, "opacity");
  ...
  connect(animation, &QPropertyAnimation::finished, this, &LoginScreen::cleanupAnimation);
